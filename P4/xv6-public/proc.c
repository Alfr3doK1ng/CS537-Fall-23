...
scheduler(void)
{
  ...
  for(;;){
    ...
    // struct proc *highPrioProc = 0;  // Initialize as null pointer
    int highestPrio = 10000000;
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      ...
      if(p->priority == highestPrio){
        ...
        // cprintf("current total ticks, pid, current_ticks: %d %d %d\n", ticks, p->pid, p->undecayed_ticks);
